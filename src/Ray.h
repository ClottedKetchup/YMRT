...
#define	MAX_BOUNDARY_RECORD 8
	...
	struct BoundaryRecord 
	{
		...
		__device__ __host__ inline BoundaryRecord() :prim_idx(0), ior(1.0f), ior_priority(0), vol_idx(-1) {}
	};

	// this function find the record that have the same prim_idx in boundary_records.
	__device__ __host__ inline int FindBoundaryPrim(const BoundaryRecord boundary_records[], const int boundary_record_count, const uint32_t prim_idx)
	{
		for (int record_idx = 0; record_idx < boundary_record_count; ++record_idx) {
	...
	// delete the record specified by record_idx.
	__device__ __host__ inline bool DeleteBoundaryRecord(BoundaryRecord boundary_records[], int *boundary_record_count, const int record_idx)
	{
		if (record_idx < 0 || record_idx >(*boundary_record_count) - 1) {
			return false;
		}
		for (int rec_idx = record_idx + 1; rec_idx < (*boundary_record_count); ++rec_idx) {
			boundary_records[rec_idx - 1] = boundary_records[rec_idx];
		}
		*boundary_record_count = (*boundary_record_count) - 1;
		return true;
	}

	__device__ __host__ inline int FindMaxPriorityRecord(const BoundaryRecord boundary_records[], const int boundary_record_count)
	{
		if (boundary_record_count == 0) {
			return -1;
		}
		float max_priority = boundary_records[0].ior_priority;
