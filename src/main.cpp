...
void TestScene_CornellBox(const std::string &exec_path, std::shared_ptr<YumeRT::SceneManager> scene_manager, int screen_width, int screen_height)
{
	...
	const uint32_t cube_idx = scene_manager->AddGeometry("Cube", YumeRT::GeometryData().InitCube());
	const uint32_t 	sphere_idx = scene_manager->AddGeometry("Sphere", YumeRT::GeometryData().InitSphere(1.0f));
	...
	const uint32_t fog_tex = scene_manager->AddTexture("fog_tex", YumeRT::Texture().InitNoiseTextureTurbulence(
		fog_tex_black, fog_tex_white, 0.5f, 2.0f, 0.5f, 4, 2.5f, 0.0f));
	...
	scene_manager->AddPrimInstance(cube_idx,
		...
		scene_manager->AddSurfaceMaterial("extra cube 2"),
		-1, true);
